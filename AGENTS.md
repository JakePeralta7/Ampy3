...
- **Fail-closed startup**: the app refuses to start when `APP_ENV=production` without a `SECRET_KEY` (≥32 chars) or when `REQUIRE_AUTH=true` without one — never fall back to disabling auth. `SECRET_KEY` under 32 characters is rejected in `settings.py`; the production gate lives in `main.py`'s lifespan and keys both session signing and at-rest token encryption.
