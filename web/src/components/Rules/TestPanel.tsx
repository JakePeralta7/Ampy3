...
export function TestPanel({ ruleId, onTestResult }: TestPanelProps) {
  ...
  const selectFromPicker = useCallback((track: UnmatchedTrack) => {
          ...
          <div className="mt-2 border border-border rounded-lg bg-bg-surface overflow-hidden">
            ...
            <div className="max-h-60 overflow-y-auto">
              {pickerLoading ? (
                ...
                filteredPickerTracks.map((track) => (
                  <button
                    key={track.sync_id}
                  ...
                  >
                    ...
                    <div className="text-xs text-fg-subtle mt-0.5 truncate">{track.sync_name}</div>
                  </button>
                ))
              )}
            </div>
